...
function gameVisual(g, cls='game-visual'){
  if(!g) return '';
  return g.image_url
    ? `<span class="${cls}"><img src="${esc(g.image_url)}" alt="" loading="lazy"><span>${esc(g.name)}</span></span>`
    : `<span class="${cls} no-image"><span class="game-placeholder">🎲</span><span>${esc(g.name)}</span></span>`;
}

function ensureImageStyles(){
  if(document.getElementById('game-image-styles')) return;
  const st=document.createElement('style');
  st.id='game-image-styles';
  st.textContent=`
    .game-visual{display:inline-flex;align-items:center;gap:10px;vertical-align:middle}
    .game-visual img{width:44px;height:44px;object-fit:cover;border-radius:7px;border:1px solid #e2d8ca;box-shadow:0 2px 5px rgba(0,0,0,.08);background:#fff}
    .game-visual.no-image .game-placeholder{width:44px;height:44px;display:grid;place-items:center;border-radius:7px;background:#f1eadf;font-size:22px}
    .game-image-overlay{position:fixed;inset:0;background:rgba(27,24,20,.62);display:flex;align-items:center;justify-content:center;padding:24px;z-index:9999}
    .game-image-modal{background:#fffdf9;border-radius:18px;max-width:820px;width:min(100%,820px);max-height:90vh;overflow:auto;padding:24px;box-shadow:0 20px 60px rgba(0,0,0,.3)}
    .game-image-modal h2{margin:0 0 6px}
    .game-image-modal .muted{margin-bottom:18px}
    .game-image-results{display:grid;grid-template-columns:repeat(auto-fill,minmax(150px,1fr));gap:14px}
    .game-image-choice{border:1px solid #e1d7c9;border-radius:12px;background:#fff;padding:9px;cursor:pointer;text-align:left;transition:.15s;min-height:205px}
    .game-image-choice:hover{transform:translateY(-2px);box-shadow:0 5px 18px rgba(0,0,0,.12);border-color:#d96a3b}
    .game-image-choice img{width:100%;height:150px;object-fit:contain;background:#f5f0e8;border-radius:8px}
    .game-image-choice span{display:block;font-size:12px;line-height:1.25;margin-top:7px}
    .game-image-actions{display:flex;justify-content:flex-end;gap:10px;margin-top:18px;flex-wrap:wrap}
    .game-image-loading{text-align:center;padding:35px 10px}
    .game-image-current{display:flex;align-items:center;gap:14px;margin:12px 0}
    .game-image-current img{width:70px;height:70px;object-fit:cover;border-radius:9px}
    .game-stats-list{margin-top:20px}.game-stats-list>div{display:flex;align-items:center;gap:12px;padding:8px 0;border-top:1px solid #eee}.game-stats-list h3{margin-bottom:6px}@media(max-width:600px){.game-image-results{grid-template-columns:repeat(2,1fr)}.game-image-choice img{height:125px}}
  `;
  document.head.appendChild(st);
}

async function searchGameImages(name){
  const url='https://commons.wikimedia.org/w/api.php?origin=*&action=query&generator=search'
    +'&gsrsearch='+encodeURIComponent(name+' board game')
    +'&gsrnamespace=6&gsrlimit=8&prop=imageinfo&iiprop=url%7Cmime%7Cextmetadata'
    +'&iiurlwidth=320&format=json';
  const res=await fetch(url);
  if(!res.ok) throw new Error('Recherche d’images indisponible');
  const data=await res.json();
  return Object.values(data.query?.pages||{}).map(x=>{
    const info=x.imageinfo?.[0]||{};
    const meta=info.extmetadata||{};
    return {
      id:String(x.pageid||x.title),
      title:String(x.title||'').replace(/^File:/,''),
      image_url:info.thumburl||info.url||'',
      image_thumb_url:info.thumburl||info.url||'',
      image_source:'Wikimedia Commons'
    };
  }).filter(x=>x.image_url && /\.(jpg|jpeg|png|webp)$/i.test(x.image_url.split('?')[0])).slice(0,8);
}

function chooseGameImage(name){
  ensureImageStyles();
  return new Promise(async resolve=>{
    const overlay=document.createElement('div');
    overlay.className='game-image-overlay';
    overlay.innerHTML=`<div class="game-image-modal"><h2>🖼️ Choisir la boîte</h2><p class="muted">Recherche pour « ${esc(name)} »</p><div class="game-image-loading">Recherche des images…</div><div class="game-image-actions"><button class="button secondary" id="noGameImage">Continuer sans image</button></div></div>`;
    document.body.appendChild(overlay);
    const close=value=>{overlay.remove();resolve(value)};
    overlay.querySelector('#noGameImage').onclick=()=>close(null);
    try{
      const results=await searchGameImages(name);
      if(!results.length){
        overlay.querySelector('.game-image-loading').innerHTML='<p>Aucune image pertinente trouvée.</p>';
        return;
      }
      overlay.querySelector('.game-image-loading').outerHTML=`<div class="game-image-results">${results.map((x,i)=>`<button class="game-image-choice" data-i="${i}"><img src="${esc(x.image_thumb_url)}" alt=""><span>${esc(x.title)}</span></button>`).join('')}</div>`;
      overlay.querySelectorAll('.game-image-choice').forEach(b=>b.onclick=()=>close(results[Number(b.dataset.i)]));
    }catch(err){
      overlay.querySelector('.game-image-loading').innerHTML=`<p>La recherche d’images n’a pas pu aboutir.</p><p class="muted">${esc(err.message)}</p>`;
    }
  });
}
...
async function calendar(){
  ...
  const days=[];
  for(let k=0;k<3;k++){const m=new Date(start.getFullYear(),start.getMonth()+k,1),last=new Date(m.getFullYear(),m.getMonth()+1,0).getDate();for(let n=1;n<=last;n++)days.push(new Date(m.getFullYear(),m.getMonth(),n));}
