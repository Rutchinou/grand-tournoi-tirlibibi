...
import {getAuth,onAuthStateChanged,signInWithEmailAndPassword,createUserWithEmailAndPassword,updateProfile,signOut,deleteUser} from "https://www.gstatic.com/firebasejs/12.9.0/firebase-auth.js";
...
const MOBILE_TOURNAMENT_CSS = `
<style id="tir-mobile-tournament-css">
.tir-mobile-tournament{display:none}
@media(max-width:700px){
  .tir-desktop-tournament{display:none!important}
  .tir-mobile-tournament{display:flex;flex-direction:column;gap:10px}
  .tir-mobile-game{border:1px solid #e2d9cc;border-radius:14px;background:#fffdf9;padding:10px 10px 12px;box-shadow:0 2px 8px rgba(40,30,20,.04)}
  .tir-mobile-game-head{display:flex;align-items:center;gap:10px;text-decoration:none;color:inherit;padding-bottom:9px;border-bottom:1px solid #eee6da}
  .tir-mobile-game-head .tir-game-thumb{width:40px!important;height:50px!important;max-width:40px!important;max-height:50px!important}
  .tir-mobile-game-title{font-size:14px;line-height:1.2;font-weight:800;min-width:0;overflow-wrap:anywhere}
  .tir-mobile-players{display:grid;grid-template-columns:repeat(5,minmax(0,1fr));gap:5px;margin-top:9px}
  .tir-mobile-player{min-width:0;text-align:center;border-radius:9px;background:#f6f1e8;padding:6px 2px 7px}
  .tir-mobile-player-name{font-size:9px;line-height:1.1;font-weight:700;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;margin-top:2px}
  .tir-mobile-player-result{min-height:30px;display:flex;align-items:center;justify-content:center;flex-direction:column;gap:1px}
  .tir-mobile-player-result .rank{width:25px;height:25px;display:inline-flex;align-items:center;justify-content:center;border-radius:50%;font-size:12px}
  .tir-mobile-player-result small{font-size:8px;line-height:1}
  .tir-mobile-empty{font-size:13px;color:#8a8075}
  .tir-mobile-wrap{padding:0!important}
  .tir-mobile-wrap .card{border-radius:14px}
}
</style>`;
function ensureMobileTournamentCss(){if(!document.querySelector('#tir-mobile-tournament-css'))document.body.insertAdjacentHTML('beforeend',MOBILE_TOURNAMENT_CSS)}
...
function imageSearchScore(title, gameName, description=''){
  ...
  let s=0;
  if(t===n)s+=420;
  else if(t===`${n} (jeu)` || t===`${n} (jeu de societe)` || t===`${n} (board game)`)s+=360;
  else if(t.startsWith(n+' '))s+=180;
  else if(t.includes(n))s+=80;
  const nt=n.split(' ').filter(x=>x.length>2);
  const matched=nt.filter(w=>t.includes(w)).length;
  s+=matched*12;
  if(/\b(jeu de société|jeu de plateau|jeu de cartes|board game|tabletop game|strategy board game|card game)\b/i.test(d))s+=180;
  if(/\b(box cover|box art|cover artwork|front cover|boîte|boite|box|cover|couverture)\b/i.test(t+' '+d))s+=100;
  if(/\b(dancing|dance|person|people|woman|man|score|scoring|points|tournament|expo|event|tableau|championship|costume|cosplay|pdf|poster|affiche|festival|convention|meeting|photograph|gameplay|playthrough|setup|spilplan|spelplan|end of game)\b/i.test(t+' '+d))s-=220;
  return s;
}
...
async function searchWikipediaDirect(gameName){
  // IMPORTANT : aucune recherche Wikipedia "large". Une recherche large
  // mélange facilement films, jeux vidéo, spectacles, etc. Si la fiche
  // exacte du jeu n'est pas identifiée, on préfère ne rien proposer.
  const variants=[
    gameName,
    `${gameName} (jeu)`,
  ...
  found.forEach(x=>{
    const k=x.page_url||x.title;
    if(!unique.has(k)||x.score>unique.get(k).score)unique.set(k,x);
  });
  return [...unique.values()].sort((a,b)=>b.score-a.score);
}

async function searchCommonsFallback(gameName){
  // Secours très strict : on ne garde qu'une image dont le nom/description
  // identifie réellement le jeu ET qui est explicitement une boîte/couverture.
  const queries=[
    `"${gameName}" "box cover"`,
  ...
  for(const q of queries){
    try{
      ...
      Object.values(j.query?.pages||{}).forEach(p=>{
        ...
        const exact=hay.includes(n);
        const close=exact || (words.length && words.filter(w=>hay.includes(w)).length===words.length);
        if(!close)return;
        const explicitBox=/\b(box cover|front cover|cover art|box art|boite|boîte|cover|couverture)\b/i.test(searchable);
        const board=/\b(jeu de société|jeu de plateau|jeu de cartes|board game|tabletop game|card game)\b/i.test(searchable);
        const setup=/\b(gameplay|playthrough|setup|score|scoring|players?|components|cards|end of game|expansion)\b/i.test(searchable);
        if(!explicitBox || !board || setup)return;
        let score=imageSearchScore(title,gameName,desc+' '+cats)+140+(exact?100:0);
        const w=Number(ii.width||0),h=Number(ii.height||0),ratio=w&&h?w/h:0;
        if(ratio>1.45)score-=100;
...
async function searchGameImages(gameName){
  const wiki=await searchWikipediaDirect(gameName);
  // Une fiche Wikipedia exacte et clairement identifiée est la meilleure
  // source automatique. Sinon, on tente Commons avec des critères très stricts.
  const commons=await searchCommonsFallback(gameName);
  return [...wiki,...commons].sort((a,b)=>b.score-a.score);
}

function reliableGameImage(results,gameName){
  if(!results?.length)return null;
  const n=norm(gameName);
  const good=results.filter(r=>{
    const title=norm(r.title||'');
    const desc=norm(r.description||'');
    const exact=title===n || title===`${n} (jeu)` || title===`${n} (jeu de societe)` || title===`${n} (board game)`;
    const mention=title.includes(n) || desc.includes(n);
    return mention && r.score>=520 && (r.source==='wikipedia' || r.source==='wikimedia_commons');
  });
  if(!good.length)return null;
  good.sort((a,b)=>b.score-a.score);
  const top=good[0], second=good[1];
  // Si deux candidats concurrents sont proches, on ne choisit rien :
  // l'utilisateur doit simplement pouvoir continuer sans image.
...
function pickGameImage(gameName){
  ...
  return new Promise(async resolve=>{
    ...
    modal.innerHTML=`<div class="tir-image-box">
      <div class="tir-image-head"><div><div class="eyebrow">IMAGE DU JEU</div><h2>Recherche de la boîte de « ${esc(gameName)} »</h2><p class="muted">Je sélectionne automatiquement une boîte uniquement si le résultat est suffisamment fiable. Sinon, aucune image n’est ajoutée.</p></div></div>
    ...
    const chosen=reliableGameImage(results,gameName);
    if(!chosen){
      modal.querySelector('.tir-image-content').innerHTML=`<div class="notice">Je n’ai pas trouvé de boîte suffisamment fiable pour « ${esc(gameName)} ». Aucune image ne sera ajoutée.</div>`;
      return;
    }
    modal.querySelector('.tir-image-content').innerHTML=`<div style="display:flex;justify-content:center"><img src="${esc(chosen.url)}" alt="Boîte ${esc(gameName)}" style="max-width:260px;max-height:340px;object-fit:contain;border-radius:10px;background:#f3f0ea"></div><p class="muted" style="text-align:center;margin:12px 0 0">Boîte identifiée automatiquement.</p>`;
    // Pas de choix manuel : une image jugée fiable est utilisée automatiquement.
    setTimeout(()=>finish(chosen),450);
  });
}
...
function register(){$('#app').innerHTML=`<div class="card narrow"><div class="eyebrow">INSCRIPTION</div><h1>Je rejoins le tournoi</h1><form id="f"><label>Prénom / pseudo<input id="name" maxlength="40" required></label><label>Email<input id="email" type="email" required></label><label>Mot de passe<input id="password" type="password" minlength="6" required></label><label>Mon avatar</label><div class="avatars">${AVATARS.map((a,i)=>`<label class="avatar-choice"><input type="radio" name="avatar" value="${a}" ${i?'':'checked'}><span>${a}</span></label>`).join('')}</div><button class="button">Créer mon compte</button></form><p class="muted">Le tournoi est limité à 5 joueurs.</p></div>`;$('#f').onsubmit=async e=>{e.preventDefault();try{const name=$('#name').value.trim(),email=$('#email').value.trim().toLowerCase(),password=$('#password').value,avatar=document.querySelector('[name=avatar]:checked').value;const c=await createUserWithEmailAndPassword(auth,email,password);const existing=await getDocs(collection(db,'players'));const count=existing.docs.filter(d=>!d.data().is_admin&&d.data().email).length;if(count>=5){await deleteUser(c.user);throw Error('Les 5 places du tournoi sont déjà prises.');}await updateProfile(c.user,{displayName:name});await setDoc(doc(db,'players',c.user.uid),{id:c.user.uid,email,name,avatar,is_admin:false,created_at:now()});location.hash='/'}catch(x){$('#app').insertAdjacentHTML('afterbegin',flash(x.code==='permission-denied'?'Impossible de finaliser l’inscription pour le moment. Veuillez réessayer.':(x.message||'Une erreur est survenue.')))}}}
...
function tournament(){
  ...
  const by={};state.results.forEach(r=>(by[r.game_id]??=[]).push(r));
  const st=state.players.map(p=>({p,pts:state.results.filter(r=>r.player_id===p.id).reduce((a,r)=>a+r.points,0)})).sort((a,b)=>b.pts-a.pts);
  const desktop=`<div class="card table-scroll tir-desktop-tournament"><table class="tournament"><thead><tr><th>Jeu</th>${state.players.map(p=>`<th>${p.avatar}<br>${esc(p.name)}</th>`).join('')}</tr></thead><tbody>${state.finals.map(f=>{const rs=by[f.game_id]||[];return `<tr><td><a href="#/result/${f.game_id}" class="tir-game-line">${gameImage(f.games,'tir-game-thumb')}<b>${f.position}. ${esc(f.games.name)}</b></a></td>${state.players.map(p=>{const r=rs.find(x=>x.player_id===p.id);return `<td>${r?`<span class="rank r${r.rank}">${r.rank}</span><br><small>${r.points} pt</small>`:'—'}</td>`}).join('')}</tr>`}).join('')}</tbody></table></div>`;
  const mobile=`<div class="tir-mobile-tournament">${state.finals.map(f=>{const rs=by[f.game_id]||[];return `<article class="tir-mobile-game"><a href="#/result/${f.game_id}" class="tir-mobile-game-head">${gameImage(f.games,'tir-game-thumb')}<span class="tir-mobile-game-title">${f.position}. ${esc(f.games.name)}</span></a><div class="tir-mobile-players">${state.players.map(p=>{const r=rs.find(x=>x.player_id===p.id);return `<div class="tir-mobile-player"><div>${p.avatar}</div><div class="tir-mobile-player-name" title="${esc(p.name)}">${esc(p.name)}</div><div class="tir-mobile-player-result">${r?`<span class="rank r${r.rank}">${r.rank}</span><small>${r.points} pt</small>`:`<span class="tir-mobile-empty">—</span>`}</div></div>`}).join('')}</div></article>`}).join('')}</div>`;
  $('#app').innerHTML=`<div class="page-head"><div><div class="eyebrow">MODULE 2</div><h1>Le tournoi</h1></div><span class="badge">${st.map((x,i)=>`${i+1}. ${x.p.avatar} ${esc(x.p.name)} · ${x.pts} pts`).join(' · ')}</span></div>${desktop}${mobile}`;
}
